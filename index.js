...
app.use(
  cors({
    origin: [
      ...
      "https://microloan-system.vercel.app",
    ],
    credentials: true,
...
app.post("/jwt", async (req, res) => {
  try {
    ...
    if (!process.env.ACCESS_TOKEN_SECRET) {
      ...
      return res
        .status(500)
        .send({ message: "Server Configuration Error: Token Secret missing" });
    }
...
app.get("/users", verifyToken, verifyAdmin, async (req, res) => {
  if (!usersCollection)
    return res.status(500).send({ message: "Database disconnected" });
